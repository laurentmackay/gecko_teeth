...
x=(0:(N-1))'/(N-1);
T=T0
p_local = p*(1+grad*(opts.grad_dist_func(xmesh/xmesh(end)-0.5)));
...
GAMMA = 2*run.*secrete(theta);% beta*(Iave-v);
...
dydt = [root.*x - y.*theta_dot; root.*y + x.*theta_dot;  -p_local.*Inh - div_flux];
