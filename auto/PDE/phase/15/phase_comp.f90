...
!     ---------- ----
      ...
      DOUBLE PRECISION CROSS, CROSS_POS, ABS_CROSS_POS, CROSSTHRESH, CROSSPT, CROSSOFF
      INTEGER :: N, I, IHAT, MID, N0
      LOGICAL :: LASTCROSS
      DOUBLE PRECISION, SAVE :: PI = 4d0*ATAN(1d0)
...
!~   END IF
	 ...
	 DO  I=1,N-2
		 ...
		 IF ((CROSS.GE.1).AND.(CROSS.LT.3)) THEN
		 IF (I+1.GT.MID) THEN
			PAR(25) = PAR(25)+ CHI/DBLE(N0-3)
		 ELSEIF (I+1.LT.MID) THEN
			PAR(25) =PAR(25) - CHI/DBLE(N0-3)
		 END IF
		 END IF
     ...
     IF (CROSS.NE.3d0) THEN
     PAR(25)=0d0
     END IF

	  IF (CROSSOFF.GT.0) THEN
		PAR(24)=(ABS_CROSS_POS/CROSSOFF)
	 ELSE
		PAR(24)=0d0
     END IF
!          IF (CROSS.LT.2d0) then
     
!     print *, '####START CHI#####, Ncross=', CROSS
!      DO  I=1,N-2
!		 PHI1=GETP('MXT',I,U)
!		 PHI2=GETP('MXT',I+1,U)
!		 PHI3=GETP('MXT',I+2,U)
