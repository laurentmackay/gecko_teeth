...
arguments
    ...
    opts.norm_rows (1,1) {mustBeNumericOrLogical} = false
    opts.norm_mat (1,1) {mustBeNumericOrLogical} = false
end 
i=1:N;
...
omega_0 = 1./T;
mu=1.0;
cij = coupling_strength(i,i',N,kappa,N);
if opts.no_self
...
cij
    ...
    function dydt = inner(t,sol)
        ...
        theta_dot =  omega_0 - 2*eps*(cij*secrete(theta)).*sense(theta);
        root = (mu-r.^2);
        % disp(root)
