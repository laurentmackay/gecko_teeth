...
theta0(N0+1) = mod(N0+1,2)*pi;
theta0(N0+2:end) =  theta_r;
theta0(1:N0) =  fliplr(theta_r);

% DOUBLE WAVE ICS, otherwise you are liable to get ncross>3
nextra=1;
temp=theta_r;
if mod(nextra,2)==0
    temp=fliplr(temp);
...
inds=(1+round((i-1)*d):round(i*d))

theta0(N0+1+inds)=temp(fliplr(inds));
theta0(N0+1-fliplr(inds))=temp(inds);

temp=fliplr(temp);

end

inds2=1:1+floor(N0/2-mod(N0+1,2));
% theta0(N0+1+inds)= fliplr(theta_r(inds));
% theta0(N0+1-fliplr(inds)) = theta_r(inds);
...
options=odeset('AbsTol',1e-11,'RelTol',1e-11, 'MaxStep', 2*pi/4);

rhs = cartesian_phase_inhibitor_ode_nonlocal_nondim(N, gamma, eps, kappa, no_self=no_self, norm_rows=norm_rows, norm_mat=norm_mat);
...
% IC=IC_acc';
for i = 1:tburn/tstep
[~,sol] = solve(rhs, [0, tstep], ICP , options);
ICP=sol(end,:);
end
if tburn<tstep
    sol=IC';
...
elseif eps~=0 && back
        [~,sol] = solve(@(t,y) rhs_back(t,y), [0,60], sol(end,:) , options);
end
[t_for,sol_for] = solve(rhs, tmesh, sol(end,:) , options);
...
tmin=22;
tburn=-tmin;
T_mid = oscillations_summary(sol_theta, tmesh, tmin=tmin, tmax=tmesh(end), dotSize=25, ...
    MinPeakDistance=0, chi_thresh=0.00,  MinPeakProminence=0.95, MinPeakHeight=6, border=true,  cycle_thresh=1.75, ...
    xlines=[0], phi_scatter=[], tburn=tburn, edmund_lines=true, checker=false, period=false, pad=0.02, raster=false, title=false, ...
    cycle_min=0);
...
disp(u_str(sol(end,:)))
%%
get_per_sol(sol(iskip:end,even(:)), tmesh, sol(end,:), rhs , NTST=1500, options=options, periodicity=1, MinPeakProminence=0.75);
%%
fn=['auto/ODE/phase_nonlocal/' num2str(N) '_gamma=' num2str(gamma) '_eps=' num2str(eps) '_kappa=' num2str(kappa) '_ftb.dat']
write_per_sol(sol(iskip:end,even(:)),tmesh, sol(end,:), rhs , fn, NTST=1500, options=options,  periodicity=1, MinPeakProminence=0.75);
